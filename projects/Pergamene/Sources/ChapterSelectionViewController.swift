...
class ChapterCell: UICollectionViewCell {
    ...
    private func setupViews() {
        contentView.backgroundColor = UIColor(red: 0.82, green: 0.72, blue: 0.58, alpha: 0.6) // Semi-transparent parchment tone
        contentView.layer.cornerRadius = 8
        contentView.layer.borderWidth = 1
        contentView.layer.borderColor = UIColor(red: 0.45, green: 0.35, blue: 0.25, alpha: 0.8).cgColor
        
        label.translatesAutoresizingMaskIntoConstraints = false
        label.textAlignment = .center
        label.font = UIFont(name: "Cardo-Bold", size: 20) ?? .systemFont(ofSize: 18, weight: .medium)
        label.textColor = UIColor(red: 0.15, green: 0.1, blue: 0.05, alpha: 1.0) // Dark text
        ...
        didSet {
            UIView.animate(withDuration: 0.1) {
                ...
                self.contentView.backgroundColor = self.isHighlighted ? 
                    UIColor(red: 0.75, green: 0.65, blue: 0.51, alpha: 0.8) : 
                    UIColor(red: 0.82, green: 0.72, blue: 0.58, alpha: 0.6)
            }
        }
    }
}